  ...
  <div style="max-width:560px;margin:0 auto">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
      <img src={$_q8z.thumbnail} alt=""
        style="width:40px;height:40px;border-radius:10px;object-fit:cover;flex-shrink:0;border:1px solid rgba(255,215,0,.2)" />
      <div style="flex:1;min-width:0">
        <p style="font-size:.75rem;font-weight:600;color:#FFD700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{$_q8z.title}</p>
        <p style="font-size:.65rem;color:rgba(255,246,204,.4);margin-top:2px">{$_q8z.duration}</p>
      </div>
      <div style="display:flex;gap:14px;align-items:center">
        <button on:click={_prv} style="color:rgba(255,246,204,.55);background:none;border:none;cursor:pointer;padding:4px;transition:color .15s"
          onmouseenter="this.style.color='#FFD700'" onmouseleave="this.style.color='rgba(255,246,204,.55)'">
          <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
        </button>
        <button on:click={_nxt} style="color:rgba(255,246,204,.55);background:none;border:none;cursor:pointer;padding:4px;transition:color .15s"
          onmouseenter="this.style.color='#FFD700'" onmouseleave="this.style.color='rgba(255,246,204,.55)'">
          <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M16 6h2v12h-2zm-3.5 6L4 6v12z"/></svg>
        </button>
      </div>
    </div>
    <iframe
      src="https://www.youtube.com/embed/{$_q8z.videoId}?autoplay=1&controls=1&rel=0&modestbranding=1"
      width="100%" height="52" frameborder="0"
      allow="autoplay; encrypted-media" title="player"
      style="border-radius:12px;width:100%;display:block;border:1px solid rgba(255,215,0,.1)"
    ></iframe>
  </div>
</div>
{/if}
