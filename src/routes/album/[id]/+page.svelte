...
<div style="max-width:560px;margin:0 auto;padding:20px 16px 0">
  <button on:click={() => history.back()}
    style="width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;
      background:rgba(255,215,0,.08);border:1px solid rgba(255,215,0,.15);cursor:pointer;color:rgba(255,215,0,.7);margin-bottom:18px">
  ...
  {:else if _data}
    <div style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:12px;padding:8px 0 22px">
      <img src={_data.cover} alt={_data.title} style="width:180px;height:180px;border-radius:16px;object-fit:cover;box-shadow:0 8px 32px rgba(0,0,0,.5);border:1px solid rgba(255,215,0,.12)" loading="lazy" />
      <h1 style="font-size:1.25rem;font-weight:700;color:#FFD700;margin:0;line-height:1.35">{_data.title}</h1>
      <p style="font-size:.78rem;color:rgba(255,246,204,.45);margin:0">{_data.songs?.length || 0} lagu</p>
      ...
      <button on:click={_playAll}
        style="margin-top:6px;display:flex;align-items:center;gap:8px;padding:11px 26px;border-radius:99px;
          background:linear-gradient(135deg,#FFD700,#FFC300);border:none;cursor:pointer;color:#141414;
          font-size:.82rem;font-weight:700;box-shadow:0 4px 18px rgba(255,215,0,.3)">
      ...
      {#each _data.songs as item, i}
        <div class="glass-card animate-card-up" style="border-radius:14px;padding:10px 12px;display:flex;gap:12px;align-items:center;animation-delay:{Math.min(i,12)*25}ms;
          ...
          <button on:click={() => _pl(i)}
            ...
            <div style="width:26px;flex-shrink:0;text-align:center;font-size:.72rem;font-weight:700;color:rgba(255,246,204,.3);position:relative">
            ...
            <div style="flex:1;min-width:0">
              <p style="font-size:.82rem;font-weight:700;color:{$_q8z?.videoId === item.videoId ? '#FFD700' : '#FFF6CC'};margin:0 0 3px;
                white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{item.title}</p>
              <p style="font-size:.68rem;color:rgba(255,246,204,.4);margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{item.artist}</p>
            </div>
            {#if item.duration}
              <span style="font-size:.68rem;color:rgba(255,246,204,.3);flex-shrink:0">{item.duration}</span>
            {/if}
          </button>
          <button on:click={e => _openMenu(e, item)}
            style="width:30px;height:30px;flex-shrink:0;border-radius:50%;display:flex;align-items:center;justify-content:center;
              background:transparent;border:none;cursor:pointer;color:rgba(255,246,204,.3)">
...
<style>
  .mini-spin { width: 16px; height: 16px; border-radius: 50%; border: 2px solid rgba(255,215,0,.2); border-top-color: #FFD700; animation: _mspin .7s linear infinite; }
