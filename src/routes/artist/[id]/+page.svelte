...
<div style="max-width:560px;margin:0 auto;padding:20px 16px 0">
  ...
  {:else if _data}
    <div style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:12px;padding:8px 0 26px">
      ...
      <h1 style="font-size:1.4rem;font-weight:700;color:#FFD700;margin:0">{_data.name}</h1>
      {#if _data.description}
        <p style="font-size:.75rem;color:rgba(255,246,204,.45);line-height:1.5;max-width:440px;
    ...
    {#if _data.topSongs?.length > 0}
      <div class="section-title" style="margin-bottom:12px">
        <div class="bar"></div>
        <h2 style="font-size:1.05rem;font-weight:700;color:#FFF6CC;margin:0">Lagu Populer</h2>
      </div>
          ...
          <div class="glass-card animate-card-up" style="border-radius:16px;padding:12px;display:flex;gap:12px;align-items:center;animation-delay:{Math.min(i,10)*30}ms">
            <button on:click={() => _pl(item, i, _data.topSongs.map(s => ({ ...s, author: s.artist })))}
              ...
              <div style="flex:1;min-width:0">
                <p style="font-size:.82rem;font-weight:700;color:{$_q8z?.videoId === item.videoId ? '#FFD700' : '#FFF6CC'};margin:0 0 4px;
                  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">{item.title}</p>
                {#if item.duration}
                  <p style="font-size:.68rem;color:rgba(255,246,204,.35);margin:0">{item.duration}</p>
                {/if}
              </div>
            </button>
    ...
    {#if _data.topAlbums?.length > 0}
      <div class="section-title" style="margin-bottom:12px">
        <div class="bar"></div>
        <h2 style="font-size:1.05rem;font-weight:700;color:#FFF6CC;margin:0">Album</h2>
      </div>
    ...
    {#if _data.topSingles?.length > 0}
      <div class="section-title" style="margin-bottom:12px">
        <div class="bar"></div>
        <h2 style="font-size:1.05rem;font-weight:700;color:#FFF6CC;margin:0">Single & EP</h2>
      </div>
      <div class="hscroll" style="margin-bottom:26px">
    ...
    {#if _data.similarArtists?.length > 0}
      <div class="section-title" style="margin-bottom:12px">
        <div class="bar"></div>
        <h2 style="font-size:1.05rem;font-weight:700;color:#FFF6CC;margin:0">Artis Serupa</h2>
      </div>
      <div class="hscroll" style="margin-bottom:26px">
        {#each _data.similarArtists as a}
          <button on:click={() => goto(`/artist/${a.id}`)} style="background:none;border:none;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:8px;width:96px;flex-shrink:0">
            <img src={a.cover} alt={a.title} style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,215,0,.15)" loading="lazy" />
            <span style="font-size:.7rem;font-weight:700;color:#FFF6CC;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">{a.title}</span>
          </button>
        {/each}
      </div>
    {/if}
...
<style>
  .mini-spin { width: 20px; height: 20px; border-radius: 50%; border: 2.5px solid rgba(255,215,0,.2); border-top-color: #FFD700; animation: _mspin .7s linear infinite; }
