...
export async function _getStreamUrl(videoId){if(_urlCache.has(videoId))return _urlCache.get(videoId);const r=await fetch(`/api/stream?id=${encodeURIComponent(videoId)}`);const j=await r.json();const url=j.url||null;if(url)_urlCache.set(videoId,url);return url;}
